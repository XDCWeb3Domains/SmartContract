...
abstract contract refCodeSystem is AdminControl {
	
    uint256 public _defaultDiscount = 10; // percent
	
	uint256 public _defaultReward = 20; // percent
	
	bool public _activeRefCode = true;
	
	struct RefCodeInfo
	{	
		string code;
		uint256 discount; // discount percent
		uint256 reward; // reward percent					
		address creator;
		uint expired;
		bool shared;
		bool status; // 
	...
	mapping(string => RefCodeInfo) public colRefCodeInfo;
	
	mapping(address => string[]) private userRefCodeList;

	function registerCode(string memory code) public
	{
		require(bytes(code).length != 0, "Code must be non-empty");
		
		uint256 _length = bytes(code).length;
		
		require(_length > 3, "Code must be more than 3 characters");	
		
		string memory _code = StringUtil.toLower(code);
		
		RefCodeInfo storage objRefCodeInfo = colRefCodeInfo[_code];
		
		require(bytes(objRefCodeInfo.code).length == 0, "Code already exists");
		
		RefCodeInfo memory _objRefCodeInfo;
		_objRefCodeInfo.creator = msg.sender;
		_objRefCodeInfo.discount = _defaultDiscount;
		_objRefCodeInfo.reward = _defaultReward;
		_objRefCodeInfo.code = _code;
		_objRefCodeInfo.status = true;
		_objRefCodeInfo.shared = false;
		_objRefCodeInfo.expired = block.timestamp + 365 days;
		
		colRefCodeInfo[_code] = _objRefCodeInfo;
			
		userRefCodeList[msg.sender].push(_code);
	}
	
	function setStatus(string memory code, bool status) public
	{
		string memory _code = StringUtil.toLower(code);
		
		RefCodeInfo storage objRefCodeInfo = colRefCodeInfo[_code];
		
		require(bytes(objRefCodeInfo.code).length != 0, "Code not exists");
		
		require(objRefCodeInfo.creator == msg.sender, "Not a creator");
		
		objRefCodeInfo.status = status;
	}
	
	function getCodeInfo(string memory code) public view returns (RefCodeInfo memory)
	{
		string memory _code = StringUtil.toLower(code);
		RefCodeInfo storage objRefCodeInfo = colRefCodeInfo[_code];
	...
	function getCodebyCreator(address creator) public view returns (string[] memory codes)
	{
		return userRefCodeList[creator];
	}
	
	function existCode(string memory code) public view returns (bool)
	{
		string memory _code = StringUtil.toLower(code);
		RefCodeInfo storage objRefCodeInfo = colRefCodeInfo[_code];
	...
	function addCode(string memory code, uint256 discount, uint256 reward, address creator, bool shared, bool status) public onlyOwner
	{
		string memory _code = StringUtil.toLower(code);
		RefCodeInfo memory _objRefCodeInfo;
		_objRefCodeInfo.creator = creator;
		_objRefCodeInfo.discount = discount;
		_objRefCodeInfo.reward = reward;
		_objRefCodeInfo.code = _code;
		_objRefCodeInfo.status = status;
		_objRefCodeInfo.shared = shared;
		_objRefCodeInfo.expired = block.timestamp + 365 days;
		
		colRefCodeInfo[_code] = _objRefCodeInfo;
		userRefCodeList[creator].push(_code);
	}
	
	
	function editCode(string memory code, uint256 discount, uint256 reward, bool shared, bool status) public onlyOwner
	{
		string memory _code = StringUtil.toLower(code);
		RefCodeInfo storage objRefCodeInfo = colRefCodeInfo[_code];
		if (bytes(objRefCodeInfo.code).length == 0){
			objRefCodeInfo.discount = discount;
			objRefCodeInfo.reward = reward;
			objRefCodeInfo.status = status;
			objRefCodeInfo.shared = shared;
		}
	}
	
	function removeCode(string memory code) public onlyOwner
	{
		string memory _code = StringUtil.toLower(code);
		RefCodeInfo storage objRefCodeInfo = colRefCodeInfo[_code];
		if (bytes(objRefCodeInfo.code).length != 0){
		
			address creator = objRefCodeInfo.creator;
			
			string[] memory codesCurrent = userRefCodeList[creator];
			
			delete colRefCodeInfo[_code];
			
			delete userRefCodeList[creator];

			for (uint i=0; i < codesCurrent.length; i++)
			{
				if (keccak256(abi.encodePacked(codesCurrent[i])) != keccak256(abi.encodePacked(_code)))
				{
					userRefCodeList[creator].push(codesCurrent[i]);
				}
			}
		}
	}

	function approvalCode(string memory code, bool status) public onlyOwner
	{
		string memory _code = StringUtil.toLower(code);
		RefCodeInfo storage objRefCodeInfo = colRefCodeInfo[_code];
		if (bytes(objRefCodeInfo.code).length == 0){	
	...
	function toUint256(string memory label) public pure returns(uint256)  {
        require (bytes(label).length != 0);
        return uint256(keccak256(abi.encodePacked(label)));
    }
	
	function setActiveRefCode() public onlyOwner {
        _activeRefCode = !_activeRefCode;
    }
	
	function setCodeConfig(uint256 discount, uint256 reward) public onlyOwner {
...
contract CallRegisterController is AdminControl, refCodeSystem
{
	...
	function getRewardShare(address ownerAddress) public view returns (uint256){
		uint256 reward = 0;
		uint256 _balance = IERC721(_contractRegister).balanceOf(ownerAddress);
		if (_balance > 0)
		{
			reward = 10;
	...
	function exists(string memory domain) external view returns (bool){
		uint256 tokenId = IRegisterController(_contractRegister).genTokenId(domain);
		return IRegisterController(_contractRegister).exists(tokenId);
	}
	
	function buyDomain(address to, string memory domain, string memory tld, string memory code) external payable 
	{
		...
		bool shared = false;
		if (bytes(code).length != 0 && _activeRefCode)
		{
			string memory _code = StringUtil.toLower(code);
			RefCodeInfo storage objRefCodeInfo = colRefCodeInfo[_code];
			if (objRefCodeInfo.status == true){
				if (objRefCodeInfo.discount > 0){
					discount = objRefCodeInfo.discount;
				}
				if (objRefCodeInfo.reward > 0){
					reward = objRefCodeInfo.reward;
				}
				if (objRefCodeInfo.creator != address(0)){
					creator = objRefCodeInfo.creator;
				}
				shared = objRefCodeInfo.shared;
			}
		}
		...
		if (shared == true){
			...
			{
				reward = getRewardShare(creator);
			}
		}
		
		if (reward > 0 && creator != address(0))
		{
			uint256 amountreward = (amount - amountdiscount) * reward/100;
			payable(creator).transfer(amountreward);
		}
    }
